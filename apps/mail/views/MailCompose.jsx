...
export function MailCompose({ close, mailId }) {
    const [mail, setMail] = useState(mailId ? mailService.get(mailId) : mailService.getEmptyMail)
    ...
    if (isComposeOpen)
        return (
            <form className="mail-compose" onSubmit={onSave}>
                <h2>
                    New Messages <span></span> <span onClick={onSaveToDraft}>x</span>
                </h2>
                <input
                    className="to"
